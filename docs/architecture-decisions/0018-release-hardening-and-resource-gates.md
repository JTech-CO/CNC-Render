...
- 상태: 승인·M12 릴리스 완료 — 단발성 로컬 LCP 초과는 사용자 승인된 미해결 P2로 유지
...
- 모든 실행의 원본 trace·DevTools log를 보존한다. 2026-09-19 clean-release 재현의
  6273.8 ms 초과는 유효 실패로 유지하며, 별도 진단 통과만으로 해소되었다고 판단하지 않는다.
- 2026-09-20 사용자가 이를 알려진 P2로 보존하고 최종 CI 통과 시 병합·배포하는 것을
  승인했다. 2.5초 및 고정 3회 모두 통과 기준은 변경하지 않고 원인 해결로 표현하지 않는다.
...
- 2026-09-20 릴리스: PR #13 최종 CI 35450789759 통과 후 `43db00c`로 main 병합.
  main CI/Pages 35451660934 및 공개 Pages 기능·출처 검사 2개가 통과했다.
  main LCP 773.612/783.719/782.089 ms, 공개 WASM MIME/해시는 해당 CI 산출물과 일치했다.
  상세 DoD 판정과 승인된 P2는 PROGRESS.md의 M12 릴리스 완료 절을 따른다.
  아래 기록은 이전 측정 이력이며 당시 대기 상태를 현재 릴리스 상태로 해석하지 않는다.
